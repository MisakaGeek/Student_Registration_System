package Login;
/*
 * 显示教授已经教授过的课程和已经选择的课程，只可以退课，返回上一层
 * */
...
import Login.ShowCourseGUI.windowlistener;

public class ShowCourseeqGUI extends JFrame implements ActionListener{//显示已经教授过的课程和已经选择的课程
	...
	public ShowCourseeqGUI(String name, String pw,Socket socket,String[] pretaughttable,String[] selectedtable) {
		super("显示已教授和已选择的课程");
		this.user_id = name;
		this.password = pw;
		this.socket0=socket;
      ...
      //下面设置已经选择的课程
    	    ...
    	    for(int i=0;i<selectedtable0.length/4-1;i++)
    	    {
              ...
              MyButtonRender3 mb3=new MyButtonRender3();
              jTable2.getColumnModel().getColumn(4).setCellRenderer(mb3);//表格里加按钮
              jTable2.getColumnModel().getColumn(4).setCellEditor(new MyButtonEditor3(jTable2,socket0));//表格里加按钮可以点击触发事件  
